...
type Props = {
  ...
  canWrite: boolean;
  onSaveEvent: (input: SemedAgendaEventInput) => Promise<{ error: string | null }>;
  onSaveMessage: (input: SemedUserMessageInput) => Promise<{ error: string | null }>;
  onMarkMessageRead: (messageId: string) => Promise<boolean>;
...
export default function SemedHomeOperationsPage({ user, users, events, messages, messageReads, canWrite, onSaveEvent, onSaveMessage, onMarkMessageRead, onNotify }: Props) {
  ...
  async function saveEvent(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await onSaveEvent({ userId: user.id, title: eventTitle, eventDate, type: eventType, startTime: eventTime, priority: eventPriority, reminderDays: 1, notes: eventNotes, status: "Agendado" });
  ...
  async function saveMessage(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const result = await onSaveMessage({ senderUserId: user.id, recipientUserId, subject: messageSubject, message: messageBody, priority: "Média", expiresAt: "" });
    if (result.error) return onNotify(result.error);
