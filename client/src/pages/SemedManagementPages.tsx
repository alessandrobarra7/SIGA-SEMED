...
export function GovernancePage({ onNavigate, initialTab = "tasks" }: { onNavigate: (view: ShellView) => void; initialTab?: GovernanceTab }) {
...
export function MastersPage() {
  ...
  return <section className="siga-management-page">
    <SectionHeading eyebrow="Cadastro geral" title="Cadastros" description="Estruturas institucionais organizadas para consulta, filtros e manutenção demonstrativa local." action={<button className="siga-page-primary" type="button" onClick={() => setFormOpen((open) => !open)}><Plus size={16} aria-hidden="true" />{formOpen ? "Fechar cadastro" : "Novo cadastro"}</button>} />
