...
| Banco de dados | Não há conexão com Cloudflare D1, ambiente de produção de referência ou migração de dados reais. A primeira etapa de negócio utiliza MySQL/Drizzle e tRPC para identidades de domínio, Cadastros Gerais, Agenda, Mensagens, confirmações de leitura e Lembretes; o repositório local versionado permanece como fallback do preview. |
...
| Persistência real incremental | Primeira etapa concluída: tabelas Drizzle/MySQL, migrações revisadas, procedimentos tRPC protegidos e sessão própria do domínio para Cadastros Gerais, Agenda, Mensagens, confirmações de leitura e Lembretes. O modo de banco só é ativado com sessão de domínio válida; caso contrário, as mesmas telas utilizam localStorage. |
...
| Testes mais recentes | 115 testes automatizados aprovados, além de checagem TypeScript e build de produção. |
