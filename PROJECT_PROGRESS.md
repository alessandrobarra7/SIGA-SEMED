...
| Shell e Início | Boas-vindas, agenda local, visão mensal navegável de prazos de Gestão, anotações, navegação lateral expansível e indicadores de tarefas abertas, aprovações pendentes e alertas de Gestão quando o perfil possui leitura permitida. A tipografia efetiva de Início foi migrada diretamente nos seletores de cabeçalho, indicadores, agenda, ações rápidas, cartões, legendas e prazos mensais para os tokens de Montserrat e Inter. | Regressão estrutural dos seletores reais de Início; 98 testes totais, TypeScript e build aprovados; validação autenticada específica em desktop e mobile posterior à alteração. |
