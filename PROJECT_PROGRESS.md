...
| Testes mais recentes | 97 testes automatizados aprovados, além de checagem TypeScript e build de produção. |
...
| Shell e Início | Boas-vindas, agenda local, visão mensal navegável de prazos de Gestão, anotações, navegação lateral expansível e indicadores de tarefas abertas, aprovações pendentes e alertas de Gestão quando o perfil possui leitura permitida. A tipografia efetiva de Início usa Montserrat nos títulos e números de destaque; após auditoria de fonte calculada e autorização específica, Manrope foi aplicada diretamente às legendas, descrições, dias, metadados, botões e controles internos visíveis. | Regressão estrutural dos seletores reais de Início; confirmação no sandbox de Montserrat em títulos e Manrope nas legendas, inclusive prazos mensais; 98 testes totais, TypeScript e build aprovados; validação autenticada específica em desktop e mobile posterior à alteração. |
